...
from didyoumean.dataset.word_freq import get_word_freq
from didyoumean.service.sub_some_word import (
    eng_to_thai,
    karan_pattern,
    sub_mispell_typing,
    sub_word,
...
class SpellCheckerService:
    def __init__(self):
        self.word_freq = get_word_freq()
        self.words = self.word_freq.keys()
        ...
        for i in check_in_split:
            if sub_mispell_typing(i) != i and not self.check_file_name(input_text):
                words = [
                    ...
                    for word in word_tokenize(sub_mispell_typing(i))
                ]
    ...
    def _normalize_sentence(self, input_checking: str) -> list:
        ...
        for sentence in input_checking.split():
            if re.search("[ก-์]+", sentence) and len(sentence) > 1:
                ...
                thai_words = word_tokenize(sentence)
                combined_thai_word = []
